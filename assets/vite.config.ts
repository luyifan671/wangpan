...
export default defineConfig({
  plugins: [
    ...
    {
      name: "dev-template-vars",
      transformIndexHtml(html) {
        // Replace Go template placeholders with dev defaults so HMR works in dev mode.
        // In production, the Go backend handles these replacements.
        return html
          .replace(/\{siteName\}/g, "Cloudreve Dev")
          .replace(/\{siteDes\}/g, "Cloudreve Development Server")
          .replace(/\{siteScript\}/g, "")
          .replace(/\{pwa_small_icon\}/g, "/favicon.ico")
          .replace(/\{pwa_medium_icon\}/g, "/favicon.ico")
          .replace(/var\(--defaultThemeColor\)/g, "#1976d2");
      },
    },
    {
